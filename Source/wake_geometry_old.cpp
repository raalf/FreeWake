...
void Squirt_out_Wake(const GENERAL,const PANEL *,const DVE *,DVE *);
//relaxing the wake
...

{
	...
	double xte[3],xto[3];	//vector along wing t.e., vector from t.e.to xo
	double ABS_xte;			//abs value of wing-t.e.
	double u1[3],u2[3];		//freestream velocities along edges in local frame.
	...
	for(k=0;k<info.nopanel;k++)
	{
		...
		for(j=0;j<panelPtr[k].n;j++)
		{
			...
			ABS_u = norm2(surfacePtr[element].u);
			wakePtr[span].U[0] = surfacePtr[element].U[0];
			wakePtr[span].U[1] = surfacePtr[element].U[1];
			wakePtr[span].U[2] = surfacePtr[element].U[2];

			//later, .u holds the locally induced velocity
			wakePtr[span].u[0] = surfacePtr[element].u[0];
			wakePtr[span].u[1] = surfacePtr[element].u[1];
			wakePtr[span].u[2] = surfacePtr[element].u[2];

//printf(" U %2.3lf %2.3lf %2.3lf u %2.3lf %2.3lf %2.3lf\n",surfacePtr[element].U[0],\
//surfacePtr[element].U[1],surfacePtr[element].U[2],surfacePtr[element].u[0],\
//surfacePtr[element].u[1],surfacePtr[element].u[2]);

			//delta x wing moved forward
			wakePtr[span].xsi=0.5*ABS_u*info.deltime;

//*****************************************************************************
			//computing reference point of squirted out wake element
		//*****************************************************************
			//Part 1: from surface DVE reference point to trailing edge:
			...
			Star_Glob(tempA,surfacePtr[element].nu,\
					  ...
					  surfacePtr[element].psi,wakePtr[span].xo);
					  				//function in ref_frame_transform.h

			//trailing edge pt. temporarily in wakePtr[span].xo
			vsum(surfacePtr[element].xo,wakePtr[span].xo,wakePtr[span].xo);
...
printf("xteV %2.3lf %2.3lf %2.3lf ",xte[0] ,xte[1],xte[2]);
printf("xte %2.3lf %2.3lf %2.3lf ",wakePtr[span].xo[0] ,wakePtr[span].xo[1],wakePtr[span].xo[2]);
//*****************************************************************************
			//Part 2: from trailing edge of wing (what is also the leading
		...
		//*****************************************************************
			...
			wakePtr[span].xo[0] +=  xto[0];
			wakePtr[span].xo[1] +=  xto[1];
			wakePtr[span].xo[2] +=  xto[2];

printf("xo %2.3lf %2.3lf %2.3lf\n",wakePtr[span].xo[0] ,wakePtr[span].xo[1],wakePtr[span].xo[2]);
//*****************************************************************************
...
printf("n %2.3lf %2.3lf %2.3lf ",wakePtr[span].normal[0] ,wakePtr[span].normal[1],wakePtr[span].normal[2]);
			//dihedral angle, nu,
			//tan(nu)=-(ny)/(nz)
			...
			tempS =(wakePtr[span].U[1] * cos(wakePtr[span].nu)\
				   +wakePtr[span].U[2] * sin(wakePtr[span].nu));
			wakePtr[span].psi = asin(tempS);
			...
			tempS = dot(xte,wakePtr[span].U)/ABS_xte;
			wakePtr[span].phiLE = asin(tempS);
printf("nu  %2.3lf  epsilon  %2.3lf  psi %2.3lf phiLE %2.3lf\n",wakePtr[span].nu*RtD,wakePtr[span].epsilon*RtD,wakePtr[span].psi*RtD,wakePtr[span].phiLE*RtD);
			//computing trailing edge phi
			//new method, using freestream velocities along edge  G.B. 11-5-06

			//free stream velocities along edges of DVE in local ref. frame
			Glob_Star(surfacePtr[element].u1,wakePtr[span].nu,\
					  wakePtr[span].epsilon,wakePtr[span].psi,u1);
			Glob_Star(surfacePtr[element].u2,wakePtr[span].nu,\
					  wakePtr[span].epsilon,wakePtr[span].psi,u2);
					  				//function in ref_frame_transform.h

			//difference in xsi components lengths
			tempS=(u2[0]-u1[0])*0.5*info.deltime;

			//TE sweep is:
			wakePtr[span].phiTE = atan(tempS/wakePtr[span].eta);


/*			//THIS MIGHT NOT BE QUITE RIGHT IF REF. FRAME ROTATES!!!!!
			//G.B. July 15, 2003, see also in equ_system.cpp
			if (j > 0)
			{
				tempS = (ABS_u - norm2(surfacePtr[element-1].u))\
			...
			else
			{
				...
				if (panelPtr[k].n==1)
				{
//					printf("WARNING! - Possible trouble with ");
//					printf("wake DVE trailing edge sweep!!\n");
//					printf("Check in Squirt_out_Wake in wing_geometry.cpp.\n");
					tempS=0;
				}
			}
			tempS += tan(wakePtr[span].phiLE);
			wakePtr[span].phiTE = atan(tempS);
*/
			//sweep at mid chord
			wakePtr[span].phi0=0.5*(wakePtr[span].phiLE+wakePtr[span].phiTE);

			//DVE half-span, projection of xte onto eta_star
			wakePtr[span].eta = ABS_xte*cos(wakePtr[span].phiLE);
			...
			//computes point halfway along left trailing edge of DVE
			Edge_Point(wakePtr[span].xo,wakePtr[span].nu,\
					   wakePtr[span].epsilon,wakePtr[span].psi,\
					   wakePtr[span].phiTE,-wakePtr[span].eta,\
