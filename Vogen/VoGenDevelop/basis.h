/*.KA{P 0}{Hilfsvereinbarungen}{Hilfsvereinbarungen}*/
/*.FE{P 0.1}
...
#include <stdlib.h>   /* wegen free, malloc, abs                      */
...
#if defined(LDOUBLE) &&                     /* Borland C++ 3.0 oder  */\
    (defined(BC3) || defined(MC6))          /* Microsoft C 6.0 mit    */
...
#undef min
#undef max
#define sign(x, y) (((y) < ZERO) ? -FABS(x) :     /* |x| mal Vor-  */  \
/*.IX{sign}*/                                                          \
                                    FABS(x))      /* zeichen von y */
#define min(a, b)        (((a) < (b)) ? (a) : (b))
/*.IX{min}*/
#define max(a, b)        (((a) > (b)) ? (a) : (b))
/*.IX{max}*/
#define SWAP(typ, a, b)                    /* zwei Objekte beliebi- */ \
/*.IX{SWAP}*/                                                          \
  { typ temp; temp = a; a = b; b = temp; } /* gen Typs vertauschen  */
...
#define SIGN(X,Y) \
/*.IX{SIGN}*/     \
             (((Y) < ZERO) ? -ABS(X) : ABS(X))    /* Vorzeichen von   */
                                                  /* Y mal ABS(X)     */
...
REAL * AllocVec (int n);  /* Speicher fuer REAL Vektor allokieren ....*/

void FreeVec  (REAL x[]); /* Speicher fuer REAL Vektor freigeben .....*/
...
REAL ** AllocMat        /* Speicher fuer Matrix allokieren ...........*/
                (int m, int n);

void FreeMat            /* Speicher fuer Matrix freigeben ............*/
            (int m, REAL * a[]);
